...
def MST_Prim(grafo_adj, ini):
    ...
    while True:
        ...
        # Adiciona as arestas conhecidas com excecao das que ligam a vertices ja visitados e substituindo por arestas de menor valor
        for i in grafo_adj[ini]:
        ...
        # Adiciona a aresta a MST
        MST.append([menor[0], arestas[menor[1]][1], menor[1]])
        arestas.pop(menor[1])  # Remove das arestas conhecidas
        vertices.remove(menor[1])  # Remove da lista de vertices existentes
        # Armazena o vertice encontrado para adicionar as novas arestas conhecidas
        ini = menor[1]
...
def rotule(clusters, rotulos):
    def rerotule(vertice):
        for j in rotulos.items():
            # Verifica se existem vertices rotulads com o vertice do parametro
...
def Dict_Arestas(lista):
    # Povoa grafo com as arestas existentes e ordenado por seus respectivos pesos
    list_adj = dict()
